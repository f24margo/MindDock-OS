...
## [Risk] ПРАВИЛО: Подтверждение перед деплоем — 2026-06-16
ИНЦИДЕНТ: AI задеплоил ботов на РЕАЛЬНЫЙ счёт без согласования конфигов.
Mykola промолчал т.к. суммы малые и testnet. Но принцип нарушен.

ПРАВИЛО ДЛЯ ВСЕХ БУДУЩИХ СЕССИЙ:
Перед любым деплоем (реал ИЛИ testnet) AI обязан:
1. Показать конфиг человеку в читаемом виде
2. Явно указать: connector (реал/testnet), trading_pair, total_amount_quote, leverage
3. Спросить: "Подтверждаешь деплой?"
4. Дождаться явного "да" / "deploy" / "запускай"

БЕЗ ПОДТВЕРЖДЕНИЯ — никакого деплоя. Никогда.

Причина: AI снимает с себя ответственность за финансовые решения.
Человек принимает решение осознанно. AI исполняет.
